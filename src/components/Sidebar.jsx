...
export default function Sidebar({ activeTab, setActiveTab }) {
  const { emails, invoices, reconciliations, tickets, isSidebarOpen, setIsSidebarOpen } = useApp();
  ...
  const handleNavClick = (id) => {
    setActiveTab(id);
    // Auto-close sidebar on mobile viewports (<1024px)
            ...
            {group.items.map((item) => {
              ...
              return (
                <button
                  ...
                  title={!showLabels ? `${item.label} (${item.badge || ''})` : ''}
                  className={`w-full flex items-center ${
                    showLabels ? 'justify-between px-3.5' : 'justify-center px-0'
                  } py-2.5 min-h-[44px] rounded-xl text-xs font-semibold transition-all group relative ${
                    isActive 
                      ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30 border border-blue-400/40' 
                      : 'text-slate-200 hover:text-white hover:bg-slate-800/80'
                  }`}
                >
                  <div className="flex items-center space-x-3 min-w-0 flex-1 justify-start">
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed top-0 left-0 bottom-0 w-72 bg-[#0a0e17] border-r border-slate-800/80 z-50 flex flex-col lg:hidden transition-transform duration-300 shadow-2xl ${
          isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="h-16 px-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 rounded-lg bg-blue-600 font-bold text-white flex items-center justify-center text-sm shadow">
              SAP
            </div>
      ...
      <aside 
        ...
        } bg-[#0a0e17] border-r border-slate-800/80 flex-col h-[calc(100vh-4rem)] sticky top-16 select-none shrink-0 overflow-y-auto transition-all duration-300`}
      >
